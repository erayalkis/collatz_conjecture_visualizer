...
def run_range():
  ...
  try:
    ...
    raise Exception(f"Range start value must be an integer! You passed in: {argv[2]}")
  
  limit = None
  try:
    limit = int(argv[3])
  except:
    raise Exception(f"Range limit value must be an integer! You passed in: {argv[3]}")
  limit = int(argv[3])
